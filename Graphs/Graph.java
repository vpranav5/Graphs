...
public class Graph {
   ...
   public void dijkstra(String startName) {
       ...
       // set the current start vertex to the string passed in
       currentStartVertexName = startName;
       ...
       // find and store the vertex object associated with paramter string
       Vertex startVertex  = vertices.get(startName);
       // set starting vertex's cost to 0
       startVertex.weightedCostFromStartVertex = 0;
    
       // loop through the vertices keyset and reset data
       Iterator<String> iterateMap = vertices.keySet().iterator();
       while(iterateMap.hasNext()) {
           Vertex v = vertices.get(iterateMap.next());
           if(!(v.equals(startVertex))) {
               v.reset();
           }
       }
       
       // store all the shortest paths in a queue, add the startVertex's path to begin loop
       PriorityQueue<Path> q = new PriorityQueue<Path>();
       q.add(new Path(startVertex, 0));   
       // handles loop-around condition by setting the startVertex's previous vertex to null
       startVertex.prev = null;
      
       // loop through all the shortest paths from startVertex to every other vertex
       while(!q.isEmpty()) {
    	   // get the vertex to evaluate and mark it as visited
    	   Vertex currentVertex = q.remove().dest;
           currentVertex.scratch = 1;
           // check all the edges this vertex is connected
           for(Edge currentEdge : currentVertex.adjacent) {
              Vertex destination = currentEdge.dest;     
               // Is this the first time I have seen this vertex?
               if(destination.scratch != 1){
            	   // get total cost as sum of weightedDistance
                   double sum = currentVertex.weightedCostFromStartVertex + currentEdge.cost;
                   // overwrite the vertex's cost based on newly calculated sum
                   if(sum < destination.weightedCostFromStartVertex) {
                       destination.weightedCostFromStartVertex = sum;
                       destination.prev = currentVertex;
                       // add this vertex's path to the queue of path objects
                       q.add(new Path(destination, destination.weightedCostFromStartVertex));
                   }
               }
          }
      }
   ...
   */
   public void findAllPaths(boolean weighted) {
      ...
      // variable to determine which path has the longest length
      double longestLength = 0;
  	  // loop through all the vertices in graph to determine all starting vertices
      for(String vertexName : vertices.keySet()){
    	// store the current vertex and clear it's instance variables
  		Vertex currentVertex = vertices.get(vertexName);
  		currentVertex.clearPathInfo();	
  		// loop through all the vertices in graph to determine all destination vertices
  		for(String visitVertex: vertices.keySet()){
  			// store the destination vertex that is received from the set
  			Vertex secondVertex = vertices.get(visitVertex);
  			// use unweightedShortestPath algorithm if paramater boolean is false
  			if(!weighted){
  				findUnweightedShortestPath(currentVertex.name);
  			}
  			// use dijkstra if the paths are weighted
  			else{
  				dijkstra(currentVertex.name);
  			}
  			// if destination vertex is not start vertex and has not been visited
  			if(!(secondVertex.name.equals(currentVertex.name)) && (secondVertex.weightedCostFromStartVertex != INFINITY)){
  				// increment number of vertex connections if this vertex is unvisited and reachable
  				currentVertex.numVertexConnected++;
  				// store the shortest path to visit current vertex from start
  				Path currentShortest = getPath(visitVertex);
  				double edgeLength = currentShortest.weightedCostOfPath;
  				
  				//unweighted path cost and weighted path cost will be the same if boolean parameter is false
  				if(!weighted){
  					currentVertex.totalUnweightedPathLength += edgeLength;
  					currentVertex.totalWeightedPathLength  += edgeLength;

  				}
  				else{
  					// consider weighted edges if the paramter is true
  					// calculate unweighted lengths based on edge lengths
  					currentVertex.totalUnweightedPathLength += edgeLength;
  					currentVertex.totalWeightedPathLength += getWeightedCostFromStart(visitVertex);
  				}					
  				// reset the longest length if the current shortest path is longer
  				if(edgeLength > longestLength){
  					longestLength = edgeLength;
  					longest = currentShortest;
  				}
  			}
  		}
  	 }   
